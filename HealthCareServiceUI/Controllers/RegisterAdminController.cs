...
namespace HealthCareServiceUI.Controllers
{
    public class RegisterAdminController : Controller
    {
        ...
        public JsonResult SaveData(SiteAdmin model)
        {
            model.IsValid = false;
            db.SiteAdmins.Add(model);
            db.SaveChanges();
        ...
        {
            SiteAdmin Data = db.SiteAdmins.Where(x => x.ID == regId).FirstOrDefault();
            Data.IsValid = true;
            db.SaveChanges();
        ...
        {
            string body = System.IO.File.ReadAllText(HostingEnvironment.MapPath("~/Views/RegisterAdmin/") + "Text" + ".cshtml");
            var regInfo = db.SiteAdmins.Where(x => x.ID == regID).FirstOrDefault();
            var url = "https://localhost:44302/" + "RegisterAdmin/Confirm?regId=" + regID;
        ...
        public JsonResult CheckValidUser(SitePatient model)
        {
            ...
            string result = "Fail";
            var DataItem = db.SiteAdmins.Where(x => x.Email == model.Email && x.Password == model.Password && x.IsValid == true).SingleOrDefault();
            if (DataItem != null)
            {
