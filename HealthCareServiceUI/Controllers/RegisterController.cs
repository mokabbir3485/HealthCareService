...
namespace HealthCareServiceUI.Controllers
{
    public class RegisterController : Controller
    {
        ...
        {
            string body = System.IO.File.ReadAllText(HostingEnvironment.MapPath("~/Views/Register/") + "Text" + ".cshtml");
            var regInfo = db.SiteUsers.Where(x => x.ID == regID).FirstOrDefault();
            var url = "https://localhost:44302/" + "Register/Confirm?regId=" + regID;
        ...
        public JsonResult CheckValidUser(SiteUser model)
        {
            ...
            string result = "Fail";
            var DataItem = db.SiteUsers.Where(x => x.Email == model.Email && x.Password == model.Password && x.IsValid == true).SingleOrDefault();
            if (DataItem != null)
            {
